    ...
    <div className="bg-white rounded-3xl shadow-xl shadow-gray-200/50 border border-gray-100 overflow-hidden">
      ...
      <div className="px-8 py-6 border-b border-gray-100 flex flex-col lg:flex-row lg:items-center gap-6 justify-between">
        <div className="flex flex-col sm:flex-row sm:items-center gap-4 lg:gap-8">
            <h2 className="text-xl font-black text-gray-900 tracking-tight">Đơn hàng</h2>
            ...
            {isLoading ? (
              ...
              orders.map((order: any) => (
                <tr key={order._id} className="hover:bg-gray-50/50 transition-colors group">
                  ...
                  <td className={`px-8 py-6 font-black ${filterType === "bus" ? "text-gray-900" : "text-gray-900"}`}>
                    <div className="flex items-center gap-2">
                        <div className={`p-1.5 rounded-lg ${filterType === "bus" ? "bg-blue-50 text-blue-600" : "bg-green-50 text-green-600"}`}>
                            {filterType === "bus" ? <Bus size={14} /> : <Home size={14} />}
                        </div>
                        {order.brand}
                    </div>
                  </td>
                  <td className="px-8 py-6 text-gray-600">
                    {filterType === "bus" ? (
                      ...
                      <div className="flex flex-col">
                        <span className="font-bold text-gray-800">{order.propertyLocation || "?"}</span>
                  ...
                  <td className="px-8 py-6 font-black text-gray-900 text-base">
                    {order.priceTicket.toLocaleString()}đ
                  </td>
                ...
                <td colSpan={6} className="px-8 py-24 text-center">
                  <div className="flex flex-col items-center gap-5 text-gray-300">
                    <div className="p-6 bg-gray-50 rounded-full">
                        {filterType === "bus" ? <Bus size={48} className="opacity-20" /> : <Home size={48} className="opacity-20" />}
                    </div>
                    <p className="font-bold text-gray-400">Bạn chưa có đơn {filterType === "bus" ? "xe khách" : "homestay"} nào.</p>
                  </div>
                </td>
              </tr>
            )}
      ...
      {!isLoading && orders.length > 0 && (
          <div className="px-8 py-4 bg-gray-50/30 border-t border-gray-50 flex items-center justify-between text-[11px] font-bold text-gray-400 uppercase tracking-widest">
              ...
              <span>Trang {currentPage} / {meta?.totalPage || 1}</span>
          </div>
      )}
              ...
              <div className="flex items-center gap-1.5 overflow-x-auto px-2">
                {Array.from({ length: meta.totalPage }, (_, i) => i + 1).map((page) => {
                    ...
                    if (
                        ...
                        return (
                        <span key={`dots-${page}`} className="flex items-center justify-center w-11 h-11 text-gray-300 font-black">
                            ...
                        </span>
                        );
                    }
...
function StatusBadge({ status }: { status: string }) {
  const styles: Record<string, string> = {
    approved: "bg-green-100 text-green-700 border border-green-200 shadow-sm shadow-green-200/50",
    completed: "bg-green-100 text-green-700 border border-green-200 shadow-sm shadow-green-200/50",
    pending: "bg-yellow-100 text-yellow-700 border border-yellow-200 shadow-sm shadow-yellow-200/50",
    rejected: "bg-red-100 text-red-600 border border-red-200 shadow-sm shadow-red-200/50",
    cancelled: "bg-red-100 text-red-600 border border-red-200 shadow-sm shadow-red-200/50",
  };
  ...
  return (
    <span className={`px-4 py-1.5 text-[10px] uppercase font-black rounded-full tracking-wider ring-1 ring-inset ${styles[status] || styles.pending}`}>
        <div className="flex items-center gap-1.5">
            <div className={`w-1.5 h-1.5 rounded-full ${dotColor[status] || dotColor.pending}`}></div>
